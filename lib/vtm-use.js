...
var THEME_VIM_PATH = buildPath('.vim');
var THEME_VIMRC_PATH = buildPath('.vimrc');
...
if (theme === 'system') {
    ...
    backup();
    clean();
    shell.cp('-r', buildPath('themes', theme, 'vim', '*'), THEME_VIM_PATH);
    shell.cp(buildPath('themes', theme, 'vimrc'), THEME_VIMRC_PATH);
    shell.cp('-r', buildPath('plugins', 'bundle', '*'), path.join(THEME_VIM_PATH, 'bundle'));
